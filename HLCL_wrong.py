...
def union(a,b):
    ...
    return coalesce(un)
# def union(a,b):
#     k = torch.tensor(list(set(tuple(i) for i in torch.cat((a, b)).cpu().numpy())))
#     return k
...
def get_arguments(): 
    ...
    parser.add_argument('--intraview_negs' ,type=str, default="simple", help='hidden dimension in the model')
    ...
    parser.add_argument('--neg', type=str, default="simple", help='number of distinct runs')
    parser.add_argument('--num_layer', type=int, default=2, help='number of layers')
    parser.add_argument('--device', type=int, default=1)
    parser.add_argument('--low_k', type=int, default=1)
    parser.add_argument('--high_k', type=int, default=1)
    parser.add_argument('--low_aug_only', action='store_true')
    args, unknown = parser.parse_known_args()
    return args

def edge_create(x, edge_index, high_k, low_k, device):
    graph = x @ x.t()
    adj_idx = to_dense_adj(edge_index)[0]
    graph = graph*adj_idx
    indices = graph.topk(k=low_k)[1]
    ...
    low_edge_index = dense_to_sparse(low_edge_index)[0]
    graph[graph==0] = 10
    indices = graph.topk(k=high_k, largest=False)[1]
    ...
    high_edge_index = dense_to_sparse(high_edge_index)[0]
    return low_edge_index, high_edge_index
...
class Encoder(torch.nn.Module):
    ...
    def forward(self, x, edge_index, high_k, low_k, device, lp_edge_index, hp_edge_index, lp_edge_weight=None, hp_edge_weight=None, edges = False):
        ...
        if edges:
            low_pass_edges = edge_create(z1, edge_index, high_k, low_k, device)
            high_pass_edges = edge_create(z1, edge_index, high_k, low_k, device)
            return z, z1, z2, union(low_pass_edges[0], high_pass_edges[0]), union(low_pass_edges[1],high_pass_edges[1])
        else:
            return z, z1, z2
...
def train(encoder_model, contrast_model, x, edge_index, high_k, low_k, device, lp_edge_index, hp_edge_index, lp_edge_weight, hp_edge_weight, optimizer, neg_mask = None, edges=False):
    encoder_model.train()
    optimizer.zero_grad()
    if edges:
        z, z1, z2, low_edge_index, high_edge_index = encoder_model(x, edge_index, high_k, low_k, device, lp_edge_index, hp_edge_index,lp_edge_weight, hp_edge_weight, edges=True)
    else:
        z, z1, z2 = encoder_model(x, edge_index, high_k, low_k, device, lp_edge_index, hp_edge_index,lp_edge_weight, hp_edge_weight, edges= False)
    ...
    optimizer.step()
    if edges:
        return loss.item(), low_edge_index.to(device), high_edge_index.to(device)
    else:
        return loss.item()


def test(encoder_model, x, edge_index, high_k, low_k, device,lp_edge_index, hp_edge_index, lp_edge_weight, hp_edge_weight, y, split):
    encoder_model.eval()
    z, _, _= encoder_model(x, edge_index, high_k, low_k, device,lp_edge_index, hp_edge_index,lp_edge_weight, hp_edge_weight, edges= False)
...
for run in range(args.runs):
    ...
    low_edge_index, high_edge_index = edge_create(data.x, data.edge_index, high_k, low_k, device)
    neg_sample=None
    ...
    with tqdm(total=preepochs, desc='(T)') as pbar:
        for epoch in range(preepochs):
            if epoch % args.per_epoch == 0 and epoch >= args.per_epoch:
                loss, low_edge_index, high_edge_index = train(encoder_model, contrast_model, data.x, data.edge_index, high_k, low_k, device, low_edge_index, high_edge_index, data.edge_weight, data.edge_weight, optimizer, neg_sample,True)
            else:
                loss = train(encoder_model, contrast_model, data.x, data.edge_index, high_k, low_k, device, low_edge_index, high_edge_index, data.edge_weight, data.edge_weight, optimizer, neg_sample,False)
            pbar.set_postfix({'loss': loss})
            pbar.update()
            if epoch % args.pre_eval == 0 and epoch >= args.pre_eval:
                test_result = test(encoder_model, data.x, data.edge_index, high_k, low_k, device, low_edge_index, high_edge_index, data.edge_weight, data.edge_weight, data.y, split)
...
with open('./results/nc_{}_wrong_HLCL_{}.csv'.format(args.dataset, args.neg), 'a+') as file:
    file.write('\n')
