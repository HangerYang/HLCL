...
def load_reddit_dataset():
    torch_dataset = Reddit(f'{DATAPATH}/Reddit')
    ...
    dataset.get_idx_split = reddit_orig_split
    dataset.label = label
...
def load_products_dataset():
    ogb_dataset = NodePropPredDataset(name='ogbn-products')
    dataset = NCDataset('ogbn-products')

    def product_orig_split(**kwargs):
    ...
    dataset.get_idx_split = product_orig_split
